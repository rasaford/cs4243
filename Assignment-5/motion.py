...
def lucas_kanade(img1, img2, keypoints, window_size=9):
    ...
    for idx, (y, x) in enumerate(keypoints):
        ...
        b = -np.ravel(It[y-w:y+w+1, x-w:x+w+1])
        v, _ = np.linalg.lstsq(A, b, rcond=None)[0]
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        for k in range(num_iters):
            ...
            if I1.shape != I2.shape:
                v = np.zeros(2)
                # print('Fix this: window out of bounds')
                break
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        p = keypoints / (scale ** L)
        d = np.array(iterative_lucas_kanade(
            pyramid1[L], pyramid2[L], p, g=g, window_size=window_size, num_iters=num_iters))
        if L != 0:
            g = (g + d) / scale
